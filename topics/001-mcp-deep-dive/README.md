...
| 实验 | 主题 | 关键收获 | 难度 | 前置 |
|------|------|----------|------|------|
| [Exp-01](experiments/01-protocol-inspector/) | 协议拦截器 | 亲眼看见 JSON-RPC 消息流，理解分帧 | ★★☆ | 无 |
| [Exp-02](experiments/02-mcp-server/) | 从零实现 Server | 掌握 Tools/Resources/Prompts 三大原语 | ★★★ | Exp-01 |
...
> - [能力协商机制](concepts/capabilities.md)
...
```
Client                          Server
  │── initialize ──────────────▶│  能力协商（版本 + capabilities）
  │◀── response ───────────────│
  │── initialized (通知) ──────▶│  确认完成
  │     ═══ 正常通信 ═══        │
  │── tools/call ──────────────▶│
...
→ [进入实验](experiments/01-protocol-inspector/)
...
→ [进入实验](experiments/02-mcp-server/)
...
→ [进入实验](experiments/03-mcp-client/)
...
→ [进入实验](experiments/04-security-lab/)
...
1. 跳读 [concepts/](concepts/) 文档，查漏补缺
2. 直接从 [Exp-02](experiments/02-mcp-server/) 开始
3. 需要调试时回到 [Exp-01](experiments/01-protocol-inspector/)

### 路径 B：系统学习（零基础）

如果你是第一次接触 MCP：
1. 阅读 [concepts/mcp-basics.md](concepts/mcp-basics.md)
2. 完成 [Exp-01](experiments/01-protocol-inspector/)（理解消息流）
3. 完成 [Exp-02](experiments/02-mcp-server/)（实现 Server）
4. 完成 [Exp-03](experiments/03-mcp-client/)（手写 Client）
5. 完成 [Exp-04](experiments/04-security-lab/)（安全攻防）

### 路径 C：安全研究（重点关注）

如果你关心 MCP 安全：
1. 快速浏览 [Exp-01](experiments/01-protocol-inspector/) 和 [Exp-02](experiments/02-mcp-server/)
2. 深入 [Exp-04](experiments/04-security-lab/)
3. 阅读 [concepts/security-model.md](concepts/security-model.md)
...
### 社区资源
- [Awesome MCP](https://github.com/pqxdev/awesome-mcp-list) - 社区 MCP Servers 列表
- [MCP 讨论区](https://github.com/modelcontextprotocol/typescript-sdk/discussions)
