 ...
 */
function checkCoverFields(doc: ParsedDocument, coverDef: any): Issue[] {
  const issues: Issue[] = [];
 ...
 */
function checkChangeLog(doc: ParsedDocument, changeLogDef: any): Issue[] {
  const issues: Issue[] = [];
 ...
 */
function findCellValue(sheet: Record<string, any>, label: string): string {
  for (const [key, value] of Object.entries(sheet)) {
    if (key.includes(label) || (value as any).label === label) {
 ...
 */
function countDataRows(sheet: Record<string, any>): number {
  let count = 0;
