 ...
 */
export async function validateLevel4(doc, template) {
    const issues = [];
 ...
 */
function checkContentDepth(doc, template) {
    const issues = [];
    for (const [sectionId, sectionDef] of Object.entries(template.sections)) {
        ...
        if (!sheet)
            continue; // Level 1 handles missing sections
 ...
 */
function measureContentLength(sheet) {
    let totalLength = 0;
 ...
 */
function isContentRepetitive(sheet, sectionLabel) {
 ...
 */
function checkSafetyRequirements(doc, template) {
    const issues = [];
 ...
 */
function checkTraceability(doc, template) {
    const issues = [];
    ...
    let sectionsWithoutTraceability = [];
    for (const [sectionId, sectionDef] of Object.entries(template.sections)) {
        if (!sectionId.startsWith('section_'))
            continue;
        const def = sectionDef;
 ...
 */
export function checkDiagrams(doc) {
    const issues = [];
