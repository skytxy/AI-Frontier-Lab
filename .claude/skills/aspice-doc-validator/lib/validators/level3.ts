 ...
 */
export async function validateLevel3(
  doc: ParsedDocument,
 ...
 */
function checkVersionConsistency(doc: ParsedDocument): Issue[] {
  const issues: Issue[] = [];
 ...
 */
function getLatestChangeLogVersion(doc: ParsedDocument): string | null {
 ...
 */
function checkChangeLogSequential(doc: ParsedDocument): Issue[] {
  const issues: Issue[] = [];
  const changeLog = doc.sheets['变更履历'];
...
function checkCrossReferences(
  ...
  for (const [sheetName, sheet] of Object.entries(doc.sheets)) {
    if (sheetName === '封面' || sheetName === '变更履历') continue;
 ...
 */
function extractReferences(sheet: Record<string, any>): string[] {
  const refs: string[] = [];
 ...
 */
function checkASPICEConsistency(
  doc: ParsedDocument,
  template: TemplateConfig
): Issue[] {
  const issues: Issue[] = [];
 ...
 */
function checkForMetricsContent(doc: ParsedDocument): boolean {
 ...
 */
function checkApprovalConsistency(doc: ParsedDocument): Issue[] {
  const issues: Issue[] = [];
  const cover = doc.sheets['封面'];

  if (!cover) return issues;
...
function extractColumnValues(sheet: Record<string, any>, headerLabel: string): string[] {
  ...
  for (const [key, value] of Object.entries(sheet)) {
    ...
    if (v.label === headerLabel || key.includes(headerLabel)) {
      ...
      values.push(v.text);
      // Stop at empty row
      if (values.length > 0 && v.text.trim() === '') break;
    }
  }
