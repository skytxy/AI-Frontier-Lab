 ...
 */
export async function validateLevel2(doc, template) {
    const issues = [];
 ...
 * Check ASPICE, ASIL, CAL level selections
 */
function checkLevelSelections(cover, metadata) {
    const issues = [];
    // Check ASPICE levels
 ...
 */
function checkChangeLogFormats(doc, changeLogDef) {
    const issues = [];
 ...
 */
function getCellValue(sheet, label) {
...
function findCellText(sheet, labelPart) {
    for (const [key, value] of Object.entries(sheet)) {
        if (key.includes(labelPart) || String(value.label || '').includes(labelPart)) {
            return value.text || String(value.raw || '');
        }
    }
    return '';
 ...
 */
function getFormatHint(fieldId) {
    const hints = {
