...
export class ASPICEValidator {
    ...
    extractMetadata(coverData) {
        ...
        for (const value of Object.values(coverData)) {
            ...
            const label = v.label;
            const text = v.text;
            if (label.includes('项目编号'))
                metadata.project_id = text;
            if (label.includes('文件编号'))
     ...
     */
    getHeaderLabel(worksheet, col, currentRow) {
    ...
    async validate(options) {
        ...
        // Run validations
        const issues = [];
        const levels = options.level === 'all' ? [1, 2, 3, 4] : [options.level];
     ...
     */
    generateSummary(issues, duration) {
        const by_severity = {
            critical: 0,
            important: 0,
            minor: 0
...
export async function main() {
    ...
    const report = validator.generateReport(result, output);
    console.log(report);
